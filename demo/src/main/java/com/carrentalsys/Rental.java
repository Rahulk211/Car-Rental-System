...
public class Rental {
    ...
    public void rentCar(int customer_id, int cars_id, int rental_days){
        ...
        String insertcar = "insert into Rented (customer_id, cars_id, rental_date, return_date, total_price) values(?,?,?,?,?)";
    ...
    private boolean isCarAvailable(int cars_id){
        ...
        try(
            ...
            if(rs.next()){
                return rs.next() && rs.getBoolean("cars_status");
            }

        }catch(SQLException e){
            throw new RuntimeException("Error checking car availability", e);
        }
    ...
    private double getCarsPrice(int cars_id){
        ...
        try(
            ...
            pstmt.setInt(1, cars_id);
            ResultSet rs = pstmt.executeQuery();
            return rs.next() ? rs.getDouble("cars_price"):0.0;
        }catch(SQLException e){
            throw new RuntimeException("Error fetching car price", e);
        }
    }
    ...
    public void returnCar(int cars_id){
        ...
        String qy = "delete * from Rented where cars_id =?";
        ...
        try(
            ...
            throw new RuntimeException(" Error returning car, e);");
        }
    }
